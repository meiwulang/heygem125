...
# [Clang 14.0.6 ]
# Embedded file name: /code/landmark2face_wy/models/networks_HD.py
# Compiled at: 2024-03-06 17:51:22
# Size of source mod 2**32: 25138 bytes
import torch
...
def define_G_gxx(input_nc, output_nc, ngf, netG, n_downsample_global=3, n_blocks_global=9, n_local_enhancers=1, n_blocks_local=3, norm="instance", gpu_ids=[], input_size=512):
    ...
    if netG == "global":
        ...
        netG = GlobalGeneratorwenet(input_nc, output_nc, ngf, n_downsample_global, n_blocks_global, norm_layer, input_size=input_size)
    elif netG == "local":
...
class GlobalGeneratoraudio(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=4, n_blocks=9, norm_layer=nn.BatchNorm2d, padding_type="reflect"):
        ...
        model1 = [
         nn.ReflectionPad2d(3), nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0), norm_layer(ngf),
         activation]
...
class GlobalGeneratorwenet(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=4, n_blocks=9, norm_layer=nn.BatchNorm2d, padding_type="reflect", input_size=512):
        assert n_blocks >= 0
                                                                                             ...
                                                                                             0)), Conv2d(64, 128, kernel_size=3, stride=(1,
                                                                                                                                         2), padding=(0,
                                                                                                                                                      0)), Conv2d(128, 128, kernel_size=3, stride=1, padding=1, residual=True), Conv2d(128, 128, kernel_size=3, stride=1, padding=1, residual=True), Conv2d(128, 256, kernel_size=3, stride=(1,
                                                                                                                                                                                                                                                                                                                                             2), padding=1), Conv2d(256, 256, kernel_size=3, stride=(int(512 / input_size)), padding=1), Conv2d(256, 256, kernel_size=3, stride=1, padding=(1,
