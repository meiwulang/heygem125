...
# [Clang 14.0.6 ]
# Embedded file name: /code/landmark2face_wy/models/base_function.py
# Compiled at: 2024-03-06 17:51:22
# Size of source mod 2**32: 14827 bytes
...
class ADAINDecoder(nn.Module):
    __doc__ = "docstring for ADAINDecoder"

    def __init__(self, pose_nc, ngf, img_f, encoder_layers, decoder_layers, skip_connect=True, nonlinearity=nn.LeakyReLU(), use_spect=False):
        ...
        for i in range(encoder_layers - decoder_layers, encoder_layers)[::-1]:
            ...
            if i != encoder_layers - 1:
                in_channels = in_channels * 2 if self.skip_connect else in_channels
...
class ADAINDecoderBlock(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, hidden_nc, feature_nc, use_transpose=True, nonlinearity=nn.LeakyReLU(), use_spect=False):
        ...
        kwargs_fine = {'kernel_size':3, 
         'stride':1,  'padding':1}
        if use_transpose:
            kwargs_up = {
              'kernel_size': 3, 'stride': 2, 'padding': 1, 'output_padding': 1}
        else:
            kwargs_up = {'kernel_size':3, 
             'stride':1,  'padding':1}
        ...
        if use_transpose:
            ...
            self.conv_1 = nn.Sequential(spectral_norm((nn.Conv2d)(hidden_nc, output_nc, **kwargs_up), use_spect), nn.Upsample(scale_factor=2))
            self.conv_s = nn.Sequential(spectral_norm((nn.Conv2d)(input_nc, output_nc, **kwargs_up), use_spect), nn.Upsample(scale_factor=2))
        self.norm_0 = ADAIN(input_nc, feature_nc)
...
class FineDecoder(nn.Module):
    __doc__ = "docstring for FineDecoder"
