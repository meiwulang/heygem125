...
# [Clang 14.0.6 ]
# Embedded file name: /code/wenet/utils/common.py
...
def add_sos_eos(ys_pad: torch.Tensor, sos: int, eos: int, ignore_id: int) -> Tuple[(torch.Tensor, torch.Tensor)]:
    ...
    Examples:
        ...
        tensor([[ 1,  2,  3,  4,  5, 11],
                ...
                [ 7,  8,  9, 11, -1, -1]])
    """
    _sos = torch.tensor([sos], dtype=(torch.long),
      requires_grad=False,
      device=(ys_pad.device))
    _eos = torch.tensor([eos], dtype=(torch.long),
      requires_grad=False,
      device=(ys_pad.device))
    ...
    return (
     pad_list(ys_in, eos), pad_list(ys_out, ignore_id))
...
def get_activation(act):
    ...
    activation_funcs = {'hardtanh':(torch.nn).Hardtanh, 
     'tanh':(torch.nn).Tanh, 
     'relu':(torch.nn).ReLU, 
     'selu':(torch.nn).SELU, 
     'swish':Swish, 
